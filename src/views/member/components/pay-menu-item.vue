<template>
  ...
  >
    <!-- 标题 -->
    <p
      class="text-base text-yellow-800"
      :class="[select ? 'dark:text-orange-300' : 'dark:text-zinc-300']"
    >
      {{ data.title }}
    </p>
    <!-- 价格 -->
    ...
    <div
      v-if="hot"
      class="absolute right-[-1px] top-[-12px] h-[22px] w-[48px] leading-[22px] text-xs text-center text-yellow-700 bg-gradient-to-r from-orange-300 to-orange-100 rounded-tr-[10px] rounded-bl-[10px]"
    >
      热销
    </div>
  </div>
